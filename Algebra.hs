...
simplify (Sum (Const a b) (Const c d))=Const (a*(b`div`(gcd b d))+c*(d`div`(gcd b d))) (gcd b d)
simplify (Mult (Const a b) (Const c d))=Const (a*c) (b*d)
...
simplify (Sum (Mult (Const a b) t1) (Mult (Const c d) t2))
    | t1==t2 = (Mult (Const (a*(b`div`(gcd b d))+c*(d`div`(gcd b d))) (gcd b d)) (simplify t1))
